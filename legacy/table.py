...
class Msg1553(object):
    ...
    def __init__(self, ch10_h5_file=None):
        ...
        if ch10_h5_file is not None:
            self.layout_version = \
                ch10_h5_file.root.Bus_Data.attrs.layout_version
        else:
    ...
    def encode_tuple(self):
        '''Take Msg1553 data and encode it into a tuple for storage in an H5
        file.
        '''
        ...
        # Version 2 - Data is stored as a variable length array of 16 bit
        # integers
...
# ---------------------------------------------------------------------------
    ...
    def decode_tuple(self, msg_tuple):
        ...
        if self.layout_version == 1:
            self.msg_time = Time.IrigTime()
            self.msg_time.time = datetime.datetime.utcfromtimestamp(msg_tuple[
                                                                    0])
    ...
    def find_msgs(self, channel_id, rt=-1, tr=-1, sa=-1, add_more=False):
        ...
        row_offsets = self.ch10_h5_file.root.Bus_Data_Index.readWhere(
            query, field="offset")
        if not add_more:
            self.row_offsets = row_offsets
        else:
            self.row_offsets = numpy.concatenate(
                (self.row_offsets, row_offsets))
    ...
    def msgs(self, index_low=0, index_high=-1):
        '''A generator that returns the specified 1553 messages one message at
        a time.
        '''
...
def import_open(irig_filename, hdf5_filename="", force=False,
                status_callback=None):
    '''Open a Ch 10 file and read it into a PyTables table or open an existing
    (i.e. already converted) table.
        irig_filename - Name of Ch 10 data file.
        hdf5_filename - Name of resultant hdf5 file.  If blank then the hdf5
            file name will be the same as the Ch 10 file but with a ".h5"
            extension.
        force - If true force a rebuild of hdf5 file, even if it exists
            already.
        status_callback - User callback function that will be called
            periodically as conversion proceeds. Will be called with one
            parameter with a value from 0.0 to 1.0 indicating how far along
            the conversion has progressed into the Ch 10 data file.
    ...
    if ch10_h5_file is None:
        ch10_h5_file = import_ch10(
            irig_filename, hdf5_filename, status_callback)
...
#            sys.exit(1)
    ...
    ch10_h5_file = tables.openFile(
        hdf5_filename, mode="w", title="Ch 10 Data File")

    # Create the 1553 message table
    if layout_version == 1:
        ch10_bus_data = ch10_h5_file.createVLArray(
            "/", "Bus_Data", tables.ObjectAtom(), title="1553 Bus Data")
    elif layout_version == 2:
        ch10_bus_data = ch10_h5_file.createVLArray(
            "/", "Bus_Data", tables.UInt16Atom(), title="1553 Bus Data")

    ch10_bus_data.attrs.layout_version = layout_version

    # Create the 1553 message index table
    ch10_bus_data_index = ch10_h5_file.createTable(
        "/", "Bus_Data_Index", IndexMsg1553, "1553 Bus Data Index")
...
# print "1553 Table Message Version
# {0}".format(ch10_h5_file.root.Bus_Data.attrs.layout_version)
    ...
    for idx in range(10):
        ...
        print "{0} Chan ID {1} {2}".format(msg_read.msg_time, msg_read.chan_id,
                                           msg_read.cmd_word_1),
        for data_index in range(0, len(msg_read.data)):
            print "0x{0:04x}".format(msg_read.data[data_index]),
        print ""

    # Demonstrate querying for a subset of messages and then interating through
    # them
    # Use this programming technique to get a subset of known messages and
    ...
    for msg in msg_read.msgs(ch10_h5_file):
        print "{0} Chan ID {1} {2}".format(msg.msg_time, msg.chan_id,
                                           msg.cmd_word_1, ),
