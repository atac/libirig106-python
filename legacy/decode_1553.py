...
def I106_Decode_First1553F1(header, msg_buffer, curr_msg):
    ret_status = Packet.IrigDataDll.enI106_Decode_First1553F1(
        ctypes.byref(header), ctypes.byref(msg_buffer), ctypes.byref(curr_msg))
    return ret_status


def I106_Decode_Next1553F1(curr_msg):
    ret_status = Packet.IrigDataDll.enI106_Decode_Next1553F1(
...
def word_cnt(CommandWord):
    if type(CommandWord) is int:
        return Packet.IrigDataDll.i1553WordCnt(
            ctypes.byref(ctypes.c_uint16(CommandWord)))
    elif type(CommandWord) is CmdWord:
        return Packet.IrigDataDll.i1553WordCnt(ctypes.byref(
            ctypes.c_uint16(CommandWord.Value)))
    else:
        return None
...
class Decode1553F1(object):
    ...
    def decode_first1553f1(self):
        RetStatus = I106_Decode_First1553F1(
            self.PacketIO.Header, self.PacketIO.Buffer,  self.CurrMsg)
        return RetStatus
    ...
    def word_cnt(self, CommandWord):
        if type(CommandWord) is int:
            ...
            return Packet.IrigDataDll.i1553WordCnt(
                ctypes.byref(ctypes.c_uint16(CommandWord.Value)))
        else:
            return None
    ...
    for PktHdr in PktIO.packet_headers():
        if PktHdr.DataType == Packet.DataType.MIL1553_FMT_1:
            ...
            for Msg in Decode1553.msgs():
                ...
                msg_time = TimeUtils.RelInt2IrigTime(
