...
def I106_SetRelTime(handle, irig_time, rel_time):
    ...
    # rel_time - 6 byte ctypes array with relative time count (usually from
    # the packet header)
    ctype_irig_time = _ctIrig106Time()
    ctype_irig_time.set_from_IrigTime(irig_time)
    ret_status = Packet.IrigDataDll.enI106_SetRelTime(
        handle, ctypes.byref(ctype_irig_time), rel_time)
    return ret_status
...
def I106_SyncTime(handle, require_sync, time_limit):
    ...
    # handle - IRIG file handle
    # require_sync - Bool indicating whether time source must be externally
    # synced to be valid
    # time_limit = Maximum amount of time in seconds to scan forward in the
    # file
...
def I106_Rel2IrigTime(handle, rel_time):
    ...
    # handle - IRIG file handle
    # rel_time - 6 byte ctypes array with relative time count (usually from the
    # packet header)
    # irig_time - Py106 time object holding absolute time
    ctype_irig_time = _ctIrig106Time()
    ret_status = Packet.IrigDataDll.enI106_Rel2IrigTime(
        handle, rel_time, ctypes.byref(ctype_irig_time))
    irig_time = IrigTime()
...
def I106_RelInt2IrigTime(handle, rel_time):
    ...
    # handle - IRIG file handle
    # rel_time - 64 integer with relative time count (usually from a message
    # header)
    # irig_time - Py106 time object holding absolute time
    ctype_rel_time = ctypes.c_uint64(rel_time)
    ctype_irig_time = _ctIrig106Time()
    ret_status = Packet.IrigDataDll.enI106_RelInt2IrigTime(
        handle, ctype_rel_time, ctypes.byref(ctype_irig_time))
    irig_time = IrigTime()
    irig_time.set_from_ctIrig106Time(ctype_irig_time)
    return ret_status, irig_time
...
class Time(object):
    ...
    def SetRelTime(self):
        ...
        # Bail out if there is not data in the packet data buffer (i.e. the
        # packet data wasn't read
        if self.PacketIO.Buffer._length_ == 0:
            return

        # Decode the time in the packet buffer
        ret_status, irig_time = MsgDecodeTime.I106_Decode_TimeF1(
            self.PacketIO.Header, self.PacketIO.Buffer)
        if ret_status != Status.OK:
            return

        I106_SetRelTime(self.PacketIO._Handle, irig_time,
                        self.PacketIO.Header.RefTime)
        return
    ...
    def Rel2IrigTime(self, rel_time):
        ...
        try:
            #            iTime = _ctIrig106Time()
            ret_status, irig_time = I106_Rel2IrigTime(
                self.PacketIO._Handle, rel_time)
            if ret_status != Status.OK:
                return None
        except Exception:
            #            traceback.print_exc()
    ...
    def RelInt2IrigTime(self, rel_time):
        ...
        try:
            #            iTime = _ctIrig106Time()
            ret_status, irig_time = I106_RelInt2IrigTime(
                self.PacketIO._Handle, rel_time)
            if ret_status != Status.OK:
                return None
        except Exception:
...
if __name__ == '__main__':
    ...
    for PktHdr in PktIO.packet_headers():
        ...
        print "'%s' %012X  ChID %3d  Data Type %-16s" % (
            PktTime, IntRefTime, PktHdr.ChID, Packet.DataType.TypeName(
                PktHdr.DataType))
